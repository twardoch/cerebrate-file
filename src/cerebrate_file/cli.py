...
def run(
    ...
    if recurse is not None:
        ...
        def process_file_wrapper(input_file: Path, output_file: Path):
            ...
            try:
                ...
                if explain:
                    # Parse frontmatter
                    metadata, content_to_chunk = parse_frontmatter_content(
                        input_content
                    )
                    logger.info(
                    ...
                    if not is_complete:
                        ...
                        temp_chunks = create_chunks(
                            content_to_chunk, data_format, chunk_size
                        )
                ...
                if progress_display:
                    progress_display.start_file(
                        str(input_file), str(output_file), len(chunks)
                    )
    ...
    if explain and "temp_chunks" in locals():
        ...
        if verbose:
            ...
            print(
                f"ℹ️  All chunks (including first) will be processed with the normal prompt"
            )
    else:
        if verbose:
    ...
    if (
        last_rate_status.headers_parsed
        and last_rate_status.requests_remaining is not None
    ):
